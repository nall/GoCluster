...
| Confidence tag looks wrong | Check current confidence docs and effective YAML ownership; for correction questions, route to spot docs, ADRs, and TSR history. | `README.md`, `spot/README.md`, `data/config/README.md`, `docs/decision-log.md`, `docs/troubleshooting-log.md` | Do not summarize an old ADR as current behavior or recommend retuning algorithm calibration without current evidence. |
...
| Path glyph is blank or weaker than expected | Confirm `SET GRID`; check `SET NOISE`, `SET PATHSAMPLES`, `SET DIAG PATH`, sample count, effective weight, stale evidence, effective path config, and YAML ownership. | `README.md`, `docs/OPERATOR_GUIDE.md`, `pathreliability/README.md`, `data/config/PATH_PREDICTIONS.md`, `data/config/README.md` | Do not call a blank glyph a bad path or recommend retuning path calibration as the first fix; it may be insufficient, low-count, low-weight, or stale evidence. |
