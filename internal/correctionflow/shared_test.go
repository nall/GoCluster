...
func TestApplyConsensusCorrectionRails(t *testing.T) {
	now := time.Unix(1700000000, 0).UTC()

	t.Run("applies when rails pass", func(t *testing.T) {
		in := testApplyInput(now, "keep")
		in.RejectInvalidBase = func(string) bool { return false }
		in.CTYValidCall = func(string) bool { return true }
		result := ApplyConsensusCorrection(in)

		if result.Suppress {
			t.Fatalf("expected no suppress")
		}
		if !result.Applied {
			t.Fatalf("expected applied correction")
		}
		if in.SpotEntry.DXCallNorm != "GOOD1" {
			t.Fatalf("expected corrected DXCallNorm GOOD1, got %q", in.SpotEntry.DXCallNorm)
		}
		if in.SpotEntry.Confidence != "C" {
			t.Fatalf("expected confidence C, got %q", in.SpotEntry.Confidence)
		}
	})

	t.Run("invalid base keep sets B", func(t *testing.T) {
		in := testApplyInput(now, "keep")
		in.RejectInvalidBase = func(string) bool { return true }
		in.CTYValidCall = func(string) bool { return true }
		result := ApplyConsensusCorrection(in)
		...
		if result.Applied {
			t.Fatalf("expected no apply")
		}
		if result.RejectReason != "invalid_base" {
			t.Fatalf("expected invalid_base reject reason, got %q", result.RejectReason)
		}
		if in.SpotEntry.Confidence != "B" {
			t.Fatalf("expected confidence B, got %q", in.SpotEntry.Confidence)
		}
	})

	t.Run("cty miss suppresses", func(t *testing.T) {
		in := testApplyInput(now, "suppress")
		in.RejectInvalidBase = func(string) bool { return false }
		in.CTYValidCall = func(string) bool { return false }
		result := ApplyConsensusCorrection(in)

		if !result.Suppress {
			t.Fatalf("expected suppress")
		}
		if result.Applied {
			t.Fatalf("expected no apply")
		}
		if result.RejectReason != "cty_miss" {
			t.Fatalf("expected cty_miss reject reason, got %q", result.RejectReason)
		}
		if in.SpotEntry.Confidence == "B" {
			t.Fatalf("expected suppress path to preserve non-B pre-rail confidence")
		}
	})
}

func testApplyInput(now time.Time, invalidAction string) ApplyInput {
	idx := spot.NewCorrectionIndex()
	window := 45 * time.Second

	others := []*spot.Spot{
		spot.NewSpotNormalized("GOOD1", "W2BBB", 14074.0, "CW"),
		spot.NewSpotNormalized("GOOD1", "W3CCC", 14074.0, "CW"),
		spot.NewSpotNormalized("GOOD2", "W4DDD", 14074.0, "CW"),
	}
	for _, other := range others {
		other.Time = now.Add(-5 * time.Second)
		other.Report = 15
		other.HasReport = true
		other.EnsureNormalized()
		idx.Add(other, now, window)
	}

	subject := spot.NewSpotNormalized("BADCALL", "W1AAA", 14074.0, "CW")
	subject.Time = now
	subject.Report = 15
	subject.HasReport = true
	subject.EnsureNormalized()
