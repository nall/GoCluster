...
- This repository keeps ADR index entries in chronological ascending order. Append new rows at the end unless the file's convention changes later.
- Preserve existing rows, links, supersession metadata, and historical ordering. Do not replace live history with starter placeholders.
