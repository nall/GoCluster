...
## How To Use
- Create new troubleshooting records in `docs/troubleshooting/` using `docs/templates/tsr-template.md`.
...
- This repository keeps TSR index entries in chronological ascending order. Append new rows at the end unless the file's convention changes later.
- Preserve existing rows, links, led-to-ADR metadata, and historical ordering. Do not replace live history with starter placeholders.
