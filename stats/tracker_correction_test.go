...
func TestTrackerObserveCallCorrectionDecision(t *testing.T) {
	tr := NewTracker()
	tr.ObserveCallCorrectionDecision("consensus", "applied", "", 1)
	tr.ObserveCallCorrectionDecision("consensus_fallback", "applied", "", 2)
	tr.ObserveCallCorrectionDecision("anchor", "rejected", "min_reports", 1)
	...
	if paths["consensus"] != 1 || paths["consensus_fallback"] != 1 || paths["anchor"] != 1 {
