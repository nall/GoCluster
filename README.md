...
- `call_correction.recent_band_bonus_*` enables a bounded recent-on-band corroboration bonus keyed by `(call, band, mode)` with unique-spotter admission over a rolling window; it only boosts `min_reports` and does not bypass advantage/confidence/freq_guard/cooldown gates.
- `call_correction.prior_bonus_*` enables a bounded `min_reports` bonus for likely-good calls (for example SCP-known calls); when combined with recent-on-band bonus it can stack up to close larger shortfalls, but it still does not bypass advantage/confidence/freq_guard/cooldown gates.
...
- `call_correction.stabilizer_*` adds an optional telnet-only delay gate for risky spots (not recently heard on the same band+mode). `stabilizer_max_checks` controls how many delay-check cycles run before timeout action; it includes the first delayed check (`1` preserves legacy single-check behavior). On timeout you can `release` or `suppress`; queue overflow is fail-open (immediate release). `P` confidence spots bypass this delay, and re-delays are skipped for `P`/`V`/`C` confidence outcomes. Delayed spots are admitted to recent-on-band support only after delay resolution; suppressed timeouts do not reinforce recent support.
