...
- Slash precedence uses existing consensus thresholds (no dedicated slash knobs): when a slash-explicit variant reaches `min_consensus_reports` inside the same base bucket, the bare form is excluded from that bucket's voting and anchor path.
