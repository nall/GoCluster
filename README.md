...
- `call_correction.stabilizer_*` adds an optional telnet-only delay gate for risky spots (not recently heard on the same band+mode). On timeout you can `release` or `suppress`; queue overflow is fail-open (immediate release). `P` confidence spots bypass this delay. Delayed spots are admitted to recent-on-band support only after delay resolution; suppressed timeouts do not reinforce recent support.
...
- One-character truncation families are controlled by `call_correction.family_policy.truncation.*`; advantage relaxation rails are configured by `call_correction.family_policy.truncation.relax_advantage.*`.
