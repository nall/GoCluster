...
7. **Call/Harmonic/License Guards** (`spot/correction.go`, `spot/harmonics.go`, `main.go`) apply call corrections, suppress harmonics, and finally run FCC license gating for DX right before broadcast/buffering (CTY validation runs in the ingest gate; corrected calls are re-validated against CTY before acceptance). Correction authority is selected by `call_correction.resolver_mode`: `shadow` keeps legacy consensus as the apply path (resolver observational), while `primary` makes resolver authoritative and keeps legacy correction shadowed for comparison. In resolver-primary mode, winner admission uses shared resolver-family gates, optional neighborhood competition (`resolver_neighborhood_*`), and the conservative one-short recent corroborator rail (`resolver_recent_plus1_*`). Harmonic suppression supports a stepped minimum dB delta (configured via `harmonics.min_report_delta_step`) so higher-order harmonics must be progressively weaker. Call correction honours `call_correction.min_snr_cw` / `min_snr_rtty` (and `min_snr_voice` if set) so marginal decodes can be ignored when counting corroborators; USB/LSB uses a wider frequency tolerance and candidate search window to reflect 3 kHz SSB bandwidth. Quality anchors are still available in legacy consensus flow, and anchor-selected winners pass the same hard gates before apply. Family behavior is configured under `call_correction.family_policy`: slash precedence uses `family_policy.slash_precedence_min_reports`, truncation matching uses `family_policy.truncation.*`, truncation advantage relaxation uses `family_policy.truncation.relax_advantage.*`, and telnet output suppression uses `family_policy.telnet_suppression.*` (including optional resolver-contested edit-neighbor suppression). `KH6/W1AW` and `W1AW/KH6` are treated as equivalent for corroboration. A legacy cooldown gate (`call_correction.cooldown_*`) can temporarily refuse to flip away from a call that already has recent diverse support unless the alternate is decisively stronger; `cooldown_min_reporters` follows `adaptive_min_reports` per band when enabled, and cooldown rejections log as `reason=cooldown` in the decision DB. A conservative split-signal ambiguity guard rejects corrections when top candidates have strong support but highly disjoint spotter sets in the same narrow frequency neighborhood (`reason=ambiguous_multi_signal`). Quality updates still reward winners, but independently validated non-winners (known callset and/or recent-on-band) are not decremented to avoid self-reinforcing penalty loops. Decision traces include `decision_path` with optional `+slash_precedence` suffix so operators can audit whether precedence filtering affected the decision path. Calls ending in `/B` (standard beacon IDs) are auto-tagged and bypass correction/harmonic/license drops (only user filters can hide them). The license gate uses a license-normalized base call (e.g., `W6/UT5UF` -> `UT5UF`) to decide if FCC checks apply and which call to query, while CTY metadata still reflects the portable/location prefix (so `N2WQ/VE3` reports Canada for DXCC but uses `N2WQ` for licensing); drops appear in the "Unlicensed US Calls" pane.
...
- You can seed call-quality anchors from your own data with `call_correction.quality_priors_file` (format: `CALL SCORE [FREQ_KHZ]`; omit/<=0 freq to apply globally). Higher scores make a call more likely to be selected as an anchor candidate in that bin, but anchor candidates still pass the same safety gates as consensus winners.
- You can optionally enable confusion-model ranking via `call_correction.confusion_model_enabled`, `call_correction.confusion_model_file`, and `call_correction.confusion_model_weight`. In legacy consensus flow this reorders tied top-support candidates; in resolver-primary flow it also resolves winner ties when top candidates are tied on weighted support and support count. Hard correction gates are unchanged.
- Confusion-model enablement example (`data/config/pipeline.yaml`):
  ...
  ```
- `call_correction.candidate_eval_top_k` lets correction evaluate multiple ranked consensus candidates (top-K) instead of only the top candidate; all existing hard gates still apply to each candidate in order.
- `call_correction.recent_band_bonus_*` enables a bounded recent-on-band corroboration bonus keyed by `(call, band, mode)` with unique-spotter admission over a rolling window. This applies to legacy consensus scoring and only boosts `min_reports`; it does not bypass advantage/confidence/freq_guard/cooldown gates.
- `call_correction.prior_bonus_*` enables a bounded `min_reports` bonus for likely-good calls (for example SCP-known calls). This applies to legacy consensus scoring and does not bypass hard gates.
- `call_correction.resolver_mode` selects correction authority: `shadow` keeps legacy correction as the apply path (resolver is observational), while `primary` makes resolver authoritative and keeps legacy correction in shadow-only comparison mode.
...
- `call_correction.stabilizer_*` adds an optional telnet-only delay gate for risky spots (not recently heard on the same band+mode) and targeted holds for resolver-ambiguous (`split`/`uncertain`), low-confidence `P`, or contested edit-neighbor cases (`stabilizer_edit_neighbor_*`). `stabilizer_max_checks` controls baseline delay-check cycles before timeout action; it includes the first delayed check (`1` preserves legacy single-check behavior). On timeout you can `release` or `suppress`; queue overflow is fail-open (immediate release). Delayed spots are admitted to recent-on-band support only after delay resolution; suppressed timeouts do not reinforce recent support.
...
Confidence glyphs are only emitted for modes that run call-correction logic (CW/RTTY/USB/LSB voice modes, whether legacy consensus apply path or resolver-primary apply path). FT8/FT4 spots carry no confidence glyphs, so confidence filters do not affect them. After correction assigns `P`/`V`/`C`/`?`, any remaining `?` is upgraded to `S` when the DX call is present in `MASTER.SCP` or has admitted recent-on-band support (same call+band+mode with sufficient unique spotters in the configured window).
...
- `cmd/rbn_replay/` keeps replay-only metrics/reporting concerns (AB metrics, run artifacts) while sharing correction policy helpers with runtime. Replay is resolver-primary only (no legacy comparison path/artifacts) and tracks resolver outcomes including neighborhood, recent-plus1, and stabilizer-delay proxy counters; see `docs/rbn_replay.md`.
...
2. Optionally enable/tune `call_correction` (master `enabled` switch, `resolver_mode`, minimum corroborating spotters, required advantage, confidence percent, recency window, max edit distance, per-mode distance models, and `invalid_action` failover). `distance_model_cw` switches CW between the baseline rune-based Levenshtein (`plain`) and a Morse-aware cost function (`morse`), `distance_model_rtty` toggles RTTY between `plain` and a Baudot/ITA2-aware scorer (`baudot`), while USB/LSB voice modes always stay on `plain` because those reports are typed by humans. Family behavior is tuned under `call_correction.family_policy` (slash precedence, truncation matching/relaxation, and telnet family suppression). Resolver-primary contested behavior is tuned with `resolver_neighborhood_*` and `resolver_recent_plus1_*`. Confusion-model behavior is controlled with `call_correction.confusion_model_enabled`, `call_correction.confusion_model_file`, and `call_correction.confusion_model_weight` (`0` disables effect). For telnet cleanup of newly-seen/busted calls, tune `call_correction.stabilizer_enabled`, `stabilizer_delay_seconds`, `stabilizer_max_checks`, `stabilizer_p_delay_confidence_percent`, `stabilizer_p_delay_max_checks`, `stabilizer_ambiguous_max_checks`, `stabilizer_edit_neighbor_*`, `stabilizer_timeout_action`, and `stabilizer_max_pending`.
...
- Call correction uses conservative baselines unless overridden: `resolver_mode=shadow`, `min_consensus_reports=4`, `family_policy.slash_precedence_min_reports=2`, `family_policy.truncation.max_length_delta=1`, `family_policy.truncation.min_shorter_length=3`, `family_policy.truncation.relax_advantage.min_advantage=0`, `family_policy.truncation.length_bonus.enabled=false`, `family_policy.truncation.delta2_rails.enabled=false`, `min_advantage=1`, `min_confidence_percent=70`, `recency_seconds=45`, `max_edit_distance=2`, `frequency_tolerance_hz=500`, `voice_frequency_tolerance_hz=2000`, `voice_candidate_window_khz=2`, `min_snr_voice=0`, `invalid_action=broadcast`, `call_quality_ttl_seconds=86400`, `call_quality_max_entries=200000`, `call_quality_cleanup_interval_seconds=600`, `call_quality_pin_priors=true`, `resolver_recent_plus1_enabled=true`, `resolver_recent_plus1_min_unique_winner=3`, `resolver_recent_plus1_require_subject_weaker=true`, `resolver_recent_plus1_max_distance=1`, `resolver_recent_plus1_allow_truncation_family=true`, `resolver_neighborhood_max_distance=1`, `resolver_neighborhood_allow_truncation_family=true`. If `resolver_neighborhood_enabled=true`, `resolver_neighborhood_bucket_radius` is clamped to `[1..2]` (otherwise `[0..2]`) and `resolver_neighborhood_max_distance<=0` is normalized to `1`. Empty distance models inherit from `distance_model` or default to `plain`; negative SNR floors/extras are clamped to zero.
