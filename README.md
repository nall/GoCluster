...
- You can optionally enable confusion-model ranking for tied top-support candidates via `call_correction.confusion_model_enabled`, `call_correction.confusion_model_file`, and `call_correction.confusion_model_weight`. This affects candidate ordering only; hard correction gates are unchanged.
...
- `cmd/rbn_replay/` keeps replay-only metrics/reporting concerns (AB metrics, run artifacts) while sharing correction policy helpers with runtime. Replay tracks resolver policy outcomes including neighborhood and recent-plus1 gate counters.
