...
- Local self-spots: when a non-test `DX` command's normalized DX call matches the logged-in callsign, the spot is treated as operator-authoritative. Resolver mutation, temporal hold, and telnet stabilizer delay are skipped; confidence is forced to `V`; peer publishing remains on the normal local-`DX` path; and correction-eligible self-spots (CW/RTTY/USB/LSB) enter `custom_scp` through the existing `V`-only runtime evidence path when enabled.
...
Confidence glyphs are normally emitted only for modes that run call-correction logic (CW/RTTY/USB/LSB voice modes). FT2/FT8/FT4/PSK/MSK144 spots normally carry no confidence glyphs, so confidence filters do not affect them. The exception is a local non-test `DX` command self-spot (`DX == DE` after normalization), which is forced to `V` and bypasses resolver/temporal/stabilizer delay. After correction assigns `P`/`V`/`C`/`?`, any remaining `?` is upgraded to `S` when the DX call is admitted by static custom-SCP membership (when `call_correction.custom_scp.enabled=true`) or admitted by recent evidence rails.
...
- `?` - Unknown/low support
- `S` - post-correction confidence would otherwise be `?` and DX call is admitted by custom-SCP static membership or recent-evidence admission
